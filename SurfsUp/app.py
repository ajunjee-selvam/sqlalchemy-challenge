...
def welcome():
    return (
        f"Welcome to the Hawaii Climate Analysis API containing data up until August 8, 2017!<br/>"
...
def precipitation():
    ...
    prcp_results= session.query(Measurement.date, Measurement.prcp).filter(Measurement.date >= prev_last_date).order_by(Measurement.date.desc()).all()
...
def stations():
    ...
    queryresult = session.query(*station_info).all()
    session.close()
